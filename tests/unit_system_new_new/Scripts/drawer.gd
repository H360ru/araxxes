...
func _draw():
	...
	if player.move_settings != null:
		if len(player.move_settings.global_pixel_path) > 0:
			draw_polyline(player.move_settings.global_pixel_path, Color.green, 2.0, true)
	...
	match control.state:
		...
		control.States.TARGETING:
			if player.aim_settings != null:
				draw_line(player.unit.global_position, player.aim_settings.global_pixel_aim, Color.red, 2.0, true)
				draw_circle(grid.get_cell_center_global(player.aim_settings.cell_to_damage), 10.0, Color.red)
