# TODO: #16 Внутреигровая командная строка
#  Мощный аддон от сообщества
#  Будем прикручивать к нему новые команды для ручных ингейм тестов
#  Возможно потребуется заняться расшиением его функционала,
#  но пока моих костылей хватает
extends CanvasLayer

const BaseCommands = preload('Misc/BaseCommands.gd')
const test = preload("res://addons/quentincaffeino-console/NewCommands.gd")
...
const CommandService = preload("res://addons/quentincaffeino-console/src/Command/CommandService.gd")#'./Command/CommandService.gd')
...
func _ready():
	...
	self.Text.connect('meta_clicked', self.Line, 'manage_meta')#'set_text')
