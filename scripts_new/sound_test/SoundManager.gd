...
# Plays a given BGM
func play_bgm(bgm : String, from_position : float = 0.0, volume_db : float = -81,\
	pitch_scale : float = -1, sound_to_override : String = ""):# -> void:
	if bgm != "" and bgm != null:
		return self.play("BGM", bgm, from_position, volume_db, pitch_scale, sound_to_override)
	elif debug:
		print_debug("No sound selected.")


# Plays a given BGS
func play_bgs(bgs : String, from_position : float = 0.0, volume_db : float = -81,\
	pitch_scale : float = -1, sound_to_override : String = ""):# -> void:
	if bgs != "" and bgs != null:
		return self.play("BGS", bgs, from_position, volume_db, pitch_scale, sound_to_override)
	elif debug:
		print_debug("No BGS selected.")


# Plays selected Sound Effect
func play_se(sound_effect : String, from_position : float = 0.0, volume_db : float = -81,\
	pitch_scale : float = -1, sound_to_override : String = ""):# -> void:
	if sound_effect != "" and sound_effect != null:
		return self.play("SE", sound_effect, from_position, volume_db, pitch_scale, sound_to_override)
	elif debug:
...
func play_me(music_effect : String, from_position : float = 0.0, volume_db : float = -81,\
	...
	if music_effect != "" and music_effect != null:
		return self.play("ME", music_effect, from_position, volume_db, pitch_scale, sound_to_override)
	elif debug:
		print_debug("No sound selected.")
	...
	if dir:
		...
		while file_name != "":
			if self.is_audio_file(file_name):
				var file_path = dir.get_current_dir().plus_file(file_name) #TODO: report issue
...
func play(sound_type : String, sound : String, from_position : float = 1.0,\
	volume_db : float = -81, pitch_scale : float = -1, sound_to_override : String = ""):# -> void:
